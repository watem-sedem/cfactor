...
def compute_crop_management_factor(grid):
    """
    #TODO
    ...
    a, Bsb, Sp, W, F, SC = compute_SC(har_tag, bdate, edate, rain, temp, p, Bsi, alpha, Ru)
...
def compute_SC(har_id, bdate, edate, rain, temp, p, Bsi, alpha, Ru):
    ...
    for i in har_id.loc[har_id != 0].unique():
        ...
        W[cond], F[cond], a_val[cond] = compute_a(rain.loc[cond], temp.loc[cond],
                                                  p.loc[cond])
        # (SG) compute Bsi
        Bsb[cond] = compute_Bsb(bdate.loc[cond], edate.loc[cond], a_val[cond],
                                Bsi.loc[cond])
...
def compute_Bsb(bdate, edate, a, Bsi):
    ...
    D = [7] + [(bdate.iloc[i] + (edate.iloc[i] - bdate.iloc[i]) /
                2 - (bdate.iloc[i - 1] + (
                        edate.iloc[i - 1] - bdate.iloc[i - 1]) / 2)).days for i in
               range(1, len(edate), 1)]
    exp = np.exp(-a * D)
